...
from simulation import model_gw150914 as gw_source
...
data = jnp.transpose(data, axes=(1,0))
data = data[None]
...
theta_true = jnp.array([[Mc, eta-0.001, chi1, chi2, dist_mpc, tc, phic]], dtype=jnp.float32)
theta_labels = ["Mc", "eta", "chi1","chi2","dist_mpc", "tc", "phic"]


""" Define mass matrix """

mass_matrix = jnp.diag(jnp.array([8.72e-02, 8.56e-05, 2.65e-02, 1.15e-02, 1.25e+01, 3.34e-05, 1.79e-02], dtype=jnp.float32))
...
chain, chain_score = sample_fn(rng, theta_initial, step_size, num_steps, burn_in=100)
