...
from simulation import model_mc_phic_dist_tc_eta as gw_source
...
dir_name = 'injection_noise_mc_phic_dist_tc_eta/'
...
tc_trigger = 1126259466.423#1126259462.423 4 SECONDS AFTER REAL SIGNAL
...
psd = jnp.array(psd/strain_scale**2, dtype=jnp_type)
...
checkpoint_dir = 'checkpoints/85/default/'
...
theta_true = jnp.array([[Mc, phic, dist_mpc, tc, eta]], dtype=jnp_type)
theta_labels = ["Mc", "phic", "dist_mpc", "tc","eta"]

noise = jnp.asarray(jnp.array([data_fd.real, data_fd.imag]) / strain_scale, dtype=jnp_type)
noise = jnp.transpose(noise, axes=(1,0))

print(jnp.sum(jnp.isnan(noise)))
...
# mass_matrix = np.linalg.cholesky(np.abs(mass_matrix))


mass_matrix = jnp.array(
[[ 1.6558579e-01,  0.0000000e+00,  0.0000000e+00,  0.0000000e+00,  0.0000000e+00],
 [ 8.7520562e-02,  5.2102581e-02,  0.0000000e+00,  0.0000000e+00,  0.0000000e+00],
 [ 2.1961987e+00, -5.4422724e-01,  3.8231127e+00,  0.0000000e+00,  0.0000000e+00],
 [ 8.0102473e-05, -8.2001985e-05,  1.5777134e-06,  7.4289062e-05,  0.0000000e+00],
 [ 3.4826717e-03, -1.7579980e-03,  1.2322246e-05,  6.2693591e-04,  1.6390748e-04]], dtype=jnp_type)
...
def wrap_score_func(data, args):
    def score_func(theta):
        score = score_likelihood_fn(theta, jnp.tile(data, (theta.shape[0],1,1)), args)
        return score
    return score_func
...
theta_initial = jnp.copy(theta_true)
rng, step_rng = jax.random.split(rng)
theta_initial = jnp.tile(theta_initial, (batch_size,1))
theta_initial += jnp.array([[0.1, 0.1, 0.1, 0.0001, 0.0001]])*jax.random.normal(rng, shape=(batch_size,5))
rng, step_rng = jax.random.split(rng)

#score_fn_jit = jax.jit(score_fn)

# theta_map = jnp.copy(theta_true)
# for i in range(100000):
#     theta_map = theta_map + 0.001*jnp.einsum('ij,bj->bi', mass_matrix @ mass_matrix.T, score_fn_jit(theta_map)) 
#     if i % 1000 == 0:
#         print(theta_map)
