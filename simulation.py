...
def model_gw150914(theta, args):
    ...
    theta = jnp.array([Mc, eta, chi1, chi2, dist_mpc, tc, phic, inclination, 0.], dtype=jnp_type)
    ...
    source = jnp.concatenate((jnp.zeros((1,2), jnp_type), source), axis=0) # Add back zero frequency
    source = jnp.nan_to_num(source)
    return jnp.asarray(source, dtype=jnp_type)
...
def model_mc_phic_dist(theta, args):
    inclination = args['inclination']
    polarization_angle = args['polarization_angle'] # polarization angle should always be zero when calling the waveform, so there's no point to this (MAX)
    f = args['f']
    f_ref = args['f_ref']
    ...
    Mc = theta[0]
    phic = theta[1]
    dist_mpc = theta[2] * strain_scale #theta[4] * strain_scale # the strain is inversely proportional to the distance, so we can just multiply the distance by the strain scale instead of dividing the strain below
...
def model_mc_phic_dist_eta(theta, args):
    inclination = args['inclination']
    polarization_angle = args['polarization_angle'] # polarization angle should always be zero when calling the waveform, so there's no point to this (MAX)
    f = args['f']
    f_ref = args['f_ref']
...
def model_mc_phic_dist_tc_eta(theta, args):
    inclination = args['inclination']
    polarization_angle = args['polarization_angle'] # polarization angle should always be zero when calling the waveform, so there's no point to this (MAX)
    f = args['f']
    f_ref = args['f_ref']
...
def model_mc_phic_dist_chi1_chi2_eta(theta, args):
    inclination = args['inclination']
    polarization_angle = args['polarization_angle'] # polarization angle should always be zero when calling the waveform, so there's no point to this (MAX)
    f = args['f']
    f_ref = args['f_ref']
